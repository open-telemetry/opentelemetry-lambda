...
import (
	...
	"github.com/open-telemetry/opentelemetry-lambda/collector/receiver/awstelemetryapi/internal/telemetryapi"
...
const scopeName = "github.com/logzio/otel-awstelemetryapi-receiver"
...
func (r *telemetryAPIReceiver) Start(ctx context.Context, host component.Host) error {
	...
	extensionID, err := apiClient.Register(ctx)
	if err != nil {
		...
		case telemetryapi.PlatformReport:
			if r.nextMetrics != nil {
				if metrics, err := r.createMetrics(e); err == nil {
					_ = r.nextMetrics.ConsumeMetrics(context.Background(), metrics)
				}
			}
		...
		case telemetryapi.Function, telemetryapi.Extension:
			if r.nextLogs != nil {
				if logs, err := r.createLogs(e); err == nil {
					_ = r.nextLogs.ConsumeLogs(context.Background(), logs)
				}
			}
		}
	}
...
func listenOnAddress(port int) string {
	...
	return "sandbox:" + strconv.Itoa(port)
}
